...
def getLoc2(I, Grid, M, P, mode):
    ...
    for n in range(N):
        ...
        Mixing_Mat[source, destination] += 1
...
if __name__ == '__main__':  
    ...
    for iter in range(1):
        ...
        L = {}
        for p in range(N):
            L[p] = (random.uniform(0, X), random.uniform(0, Y))
        ...
        while T <= duration:
            ...
            pickle.dump([daily_indv_CPs, Grid_Mean, Mixing_Mat], open("daily_CP_for_dist_"+str(N)+"_no_adj2.p", "wb"))
        ...
        pickle.dump(Mean_CPs, open("Mean_CPs_Daily_"+str(N)+"_no_adj2.p", "wb"))
        pickle.dump(status_counts, open("status_counts_"+str(N)+"_no_adj2.p", "wb"))
        pickle.dump([conf_int[0], adj_conf_int[0]], open("sampling_inf_"+str(N)+"_no_adj2.p", "wb"))
